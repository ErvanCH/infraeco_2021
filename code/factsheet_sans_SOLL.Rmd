...
```{r sumstat, eval=T, echo=F,message=F,warning=F,fig.width=6,fig.height=6,fig.pos='H',tidy.opts=list(width.cutoff=50),tidy=TRUE, fig.show='asis', results='show'}
  path <- "C:/Dossier_Ervan/R/infraeco_2021/data/infofauna"
  FF <- list.files(path)[grep(paste0("_",GUILD,"_bio-idx"),list.files(path))]
  ist <- data.table::fread(paste(path,FF,sep="/"))
  tab <- data.frame(var=c("Observed quality"),val=c(nrow(ist)))
  tab$perc <-  round(tab$val*100/4140543,2)
  
  kable(tab,format="html",align=rep('c', 1),booktabs = T,col.names=c("","# ha","% CH"),caption = "<b>Table 3: Number sites (# sites) and corresponding area (area ha). proportion of Swiss territory (4140543 ha, % CH) for  observed quality <b>") %>%
